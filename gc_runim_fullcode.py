...
class ConvergenceAnalyzer:
    ...
    def record_run(self, fitness_history: List[float]):
        cumulative_best = np.minimum.accumulate(fitness_history).tolist()
        self.history.append(cumulative_best)
    ...
    def plot_convergence(self, save_path=None):
        ...
        plt.title('Convergence Analysis (10 Independent Runs)', fontsize=14)
        plt.legend()
...
def batch_process_dataset(dataset_folder: str, hybrid_optimizer_class,
    ...
    for idx, img_folder in enumerate(image_folders, 1):
        ...
        try:
            ...
            # Save results
            if save_results:
                save_path = Path(dataset_folder) / f"results-{img_folder}"
...
all_results = batch_process_dataset(
    dataset_folder="/content/drive/MyDrive/ScientificPapers/MH-Multispectral/Dataset",
